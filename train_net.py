...
class AzureCOCOPanopticEvaluator(COCOPanopticEvaluator):
    """
    Can be used in CityScapes, COCO and ADE20K
    """
    ...
    def evaluate(self):
        ...
        if azure and result is not None:
            for key in result["panoptic_seg"].keys():
                run.log(name=key, value=result["panoptic_seg"][key])
        return result
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type in [
            ...
            if cfg.MODEL.MASK_FORMER.TEST.PANOPTIC_ON:
                ...
                evaluator_list.append(
                    AzureCOCOPanopticEvaluator(dataset_name, output_folder)
                )
...
def setup(args):
    ...
    setup_logger(
        output=cfg.LOG_TEMP_OUTPUT, distributed_rank=comm.get_rank(), name="mask2former"
    )
    return cfg
