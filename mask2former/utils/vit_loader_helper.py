...
def load_weights_from_npz(model, checkpoint_path: str, prefix: str = ""):
    ...
    _copy("patch_embed.proj.weight", embed_conv_w)
    _copy("patch_embed.proj.bias", _n2p(w[f"{prefix}embedding/bias"]))
    ...
    for i, block in enumerate(model.layers.children()):
        ...
        _copy(
            f"layers.{i}.attn.in_proj.weight",
            torch.cat(
                [
