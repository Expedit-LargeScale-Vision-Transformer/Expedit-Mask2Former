...
class TokenClusteringBlock(nn.Module):
    def __init__(self, cfg):
        ...
        assert cfg.WINDOW_SIZE % 2 == 1
        self.r = cfg.WINDOW_SIZE // 2
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention(
            ...
            attn_drop=attn_drop,
            proj_drop=drop,
            ckpt_window_size=to_2tuple(ckpt_window_size),
        )
...
class HourglassBasicLayer(BasicLayer):
    def __init__(
        self,
        dim,
        depth,
        ...
        super(HourglassBasicLayer, self).__init__(
            dim,
            depth,
            num_heads,
            window_size,
    ...
    def reconstruct(self, x, H, W, reconstructer):
        B, _, C = x.shape
        x = x.view(B, H, W, C)
        ...
        if H != reconstructer.hw_shape[0] or W != reconstructer.hw_shape[1]:
            H, W = reconstructer.hw_shape
            x = x[:, :H, :W, :].contiguous()
        x = x.view(B, -1, C)
        return x
    ...
    def forward(self, x, H, W):
        ...
        reconstructer.update_state(aspect_ratio=get_aspect_ratio(H, W))
        ...
        for i, blk in enumerate(self.blocks):
            if i == self.clustering_location:
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    x, (Wh, Ww) = self.cluster(x.float(), reconstructer)
        ...
        if x.shape[1] != np.prod(reconstructer.hw_shape) or clustered:
            x_dtype = x.dtype
            with torch.cuda.amp.autocast(enabled=False):
                x = self.reconstruct(x.float(), H, W, reconstructer)
            x = x.to(dtype=x_dtype)
            H, W = reconstructer.hw_shape
...
class HourglassSwinTransformer(SwinTransformer):
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            if clustering_location < self.layers[i_layer].depth:
...
class D2HourglassSwinTransformer(HourglassSwinTransformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        token_clustering_cfg = cfg.MODEL.SWIN.TOKEN_CLUSTERING_BLOCK
        clustering_location = cfg.MODEL.SWIN.CLUSTERING_LOCATION
        token_reconstruction_cfg = cfg.MODEL.SWIN.TOKEN_RECONSTRUCTION_BLOCK
        ...
        super().__init__(
            ...
            patch_norm,
            use_checkpoint=use_checkpoint,
            token_clustering_cfg=token_clustering_cfg,
            clustering_location=clustering_location,
            token_reconstruction_cfg=token_reconstruction_cfg,
        )
