...
class MaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            # resize
            "resize_short_edge_length": cfg.MODEL.MASK_FORMER.TEST.RESIZE_SHORT_EDGE_LENGTH,
            "resize_ratio": cfg.MODEL.MASK_FORMER.TEST.RESIZE_RATIO,
        }
    ...
    def forward(self, batched_inputs):
        ...
        if self.resize_ratio > 0:
            # short_edge_length = min(x.shape[-1], x.shape[-2])
            resize_shape = (
                int(x.shape[-2] / shape_divisable * self.resize_ratio)
                * shape_divisable,
                int(x.shape[-1] / shape_divisable * self.resize_ratio)
                * shape_divisable,
            )
            # H, W = int(ratio * x.shape[-2]), int(ratio * x.shape[-1])
        ...
        torch.cuda.synchronize()
        self.time_backbone += time.perf_counter() - start_time
        torch.cuda.synchronize()
        start_time = time.perf_counter()
        ...
        if self.training:
            ...
            # auzre loss module
            if self.curve_count == self.curve_iters:
                show_loss = (
                    losses["loss_ce"].item()
                    + losses["loss_mask"].item()
                    + losses["loss_dice"].item()
                )
                class_loss = losses["loss_ce"].item()
                matched_class_loss = losses["loss_ce_matched"].item()
                unmatched_class_loss = losses["loss_ce_unmatched"].item()
                mask_loss = losses["loss_mask"].item()
                dice_loss = losses["loss_dice"].item()
                self.curve_count = 0
            else:
                self.curve_count += 1
            ...
            # the same shape as image.tensor.shape
            ...
            torch.cuda.synchronize()
            self.time_head += time.perf_counter() - start_time
